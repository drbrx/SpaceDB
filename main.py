...
pagina = st.sidebar.selectbox("OPERAZIONI", [
...
if pagina == "Missioni per sito di lancio":
    ...
    attacco_prec = st.text_input("Codice attacco stadio precedente (facoltativo)", value="")
    if st.button("Aggiungi stadio"):
        try:
            cursor.callproc("AggiungiStadioMissione", (
    ...
    idMissione = st.number_input("ID Missione", min_value=1)
    pubblico = st.radio("Visibili al pubblico?", [True, False])
    ...
    if st.button("Aggiorna quantità"):
        try:
            cursor.callproc("ImpostaQuantitaStrumentoStadio", (
                idStadio, idStrumento, qty
            ))
            conn.commit()
